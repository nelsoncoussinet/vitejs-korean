...
import com.freyja.koreanlearning.GrammarDetailScreen
import com.freyja.koreanlearning.MainActivity

class GrammarScreen(val mainActivity: MainActivity) : IScreen
{
    @Composable
    override fun Show() {
        var selectedGrammar by remember { mutableStateOf<Grammar?>(null) }

        Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
            if (selectedGrammar == null) {
                DashboardScreen(
                    modifier = Modifier.padding(innerPadding),
                    onWordClick = { selectedGrammar = it }
                )
            } else {
                GrammarDetailScreen(
    ...
    fun WordListSection(
        ...
        val words = remember { mutableStateListOf<Grammar>() }

        LaunchedEffect(Unit) {
            try {
                val results = mainActivity.database.getAllGrammars()
                words.clear()
        ...
        val filteredWords = words.filter {
            it.grammar.contains(searchQuery, ignoreCase = true) ||
                    it.definitionFr.contains(searchQuery, ignoreCase = true)  ||
                    it.definitionKr.contains(searchQuery, ignoreCase = true)
        }
            ...
            LazyColumn(modifier = Modifier.fillMaxWidth()) {
                items(filteredWords) { word ->
                    ListItem(
                        ...
                        headlineContent = { Text(word.grammar) },
                        supportingContent = { Text(word.definitionFr) },
                        trailingContent = {
